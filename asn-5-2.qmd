...
Feedback: The variable that represents the banned period is Ban. (1 point deduction)
...
```{r}
summary(model<-lm(PIC ~ Ban + harford, data=df))
```
...
```{r}
summary(model2<-lm(PIC ~ Ban + harford + Qs_tavg, data=df))
```

Suggestion: Other numerical variables could have been included. I would suggest running the regression below instead. (0.5 point deduction)

```{r}
summary(model3<-lm(PIC ~ Ban + harford + 
                     Qs_tavg + Evap_tavg + Qs_tavg + Rainf_f_tavg + Tair_f_tavg +
                     Year, data=df))
```
...
Feedback: This is not correct. 0.038347 is the intercept. The coefficient of interest is 0.002156. (1 point deduction)
...
```{r}
summary(model4<-felm(PIC ~ Ban + Tax + harford + Qs_tavg, data=df))
```

Feedback: Your team did not include any categorical variable in the format suggested. I would suggest use Month and the county name as the categorical variable. Below is the suggested regression. (2 points deduction)

```{r}
summary(model5<-felm(PIC ~ Ban + Year +
                     Evap_tavg + Qs_tavg + Rainf_f_tavg + Tair_f_tavg |
                       Month + name, data=df))

summary(model6<-felm(PIC ~ Ban + harford + Year +
                     Evap_tavg + Qs_tavg + Rainf_f_tavg + Tair_f_tavg |
                       Month + state, data=df))
...
Feedback: 0.039424 is the intercept. The treatment effect is 0.00232. (1 point deduction)
